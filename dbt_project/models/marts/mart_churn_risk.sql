...
--         -- handle NULL activity safely
--         COALESCE(
--             DATEDIFF('day', la.last_event_ts, CURRENT_TIMESTAMP),
--             999
--         ) AS days_inactive,

--         COALESCE(fp.failed_payment_count, 0) AS failed_payment_count,
--         COALESCE(sc.support_case_count, 0)   AS support_case_count,

--         CASE WHEN c.user_id IS NOT NULL THEN TRUE ELSE FALSE END
--             AS has_cancellation
...
--     -- ✅ FIX: cap score at 3
--     LEAST(raw_score, 3) AS churn_risk_score,

--     CASE
--         WHEN raw_score >= 3 THEN 'high'
...
churn_signals AS (
    SELECT
        ...
        -- handle NULL activity safely
        COALESCE(
            DATEDIFF(CURRENT_DATE(), CAST(la.last_event_ts AS DATE)),
            999
        ) AS days_inactive,
